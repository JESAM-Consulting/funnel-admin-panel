...
const Envoltec = () => {
  ...
  const getNewsData = async () => {
    ...
    await ApiGet(`get_applyNow?project=Envoltec&page=${page}&limit=${countPerPage}`)
      .then((res) => {
  ...
  const columns = [
    ...
    },
    {
      name: "isSales",
      cell: (row) => {
        return <>{row?.isSales === true ? "Yes" : "No"}</>;
      },
      selector: "isSales",
      sortable: true,
      width: "200px",
    },
    {
      name: "workYears",
      cell: (row) => {
        return <>{row?.workYears ? row?.workYears : "-"}</>;
      },
      selector: "workYears",
      sortable: true,
      width: "200px",
